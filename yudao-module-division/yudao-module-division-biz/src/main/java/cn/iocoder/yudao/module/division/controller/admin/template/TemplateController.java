package cn.iocoder.yudao.module.division.controller.admin.template;
...
import cn.iocoder.yudao.module.division.service.template.TemplateService;

@Tag(name = "管理后台 - Excel自动化")
@RestController
@RequestMapping("/division/template")
@Validated
public class TemplateController {
    ...
    @PostMapping("/create")
    @Operation(summary = "创建Excel自动化")
    @PreAuthorize("@ss.hasPermission('division:template:create')")
    ...
    @PutMapping("/update")
    @Operation(summary = "更新Excel自动化")
    @PreAuthorize("@ss.hasPermission('division:template:update')")
    ...
    @Operation(summary = "删除Excel自动化")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('division:template:delete')")
    ...
    @GetMapping("/get")
    @Operation(summary = "获得Excel自动化")
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('division:template:query')")
    ...
    @GetMapping("/page")
    @Operation(summary = "获得Excel自动化分页")
    @PreAuthorize("@ss.hasPermission('division:template:query')")
    ...
    @Operation(summary = "导出Excel自动化 Excel")
    @PreAuthorize("@ss.hasPermission('division:template:export')")
    ...
    public void exportTemplateExcel(@Valid TemplatePageReqVO pageReqVO,
        ...
        // 导出 Excel
        ExcelUtils.write(response, "Excel自动化.xls", "数据", TemplateRespVO.class,
